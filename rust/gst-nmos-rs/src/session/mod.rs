...
pub(crate) struct CommonSettings {
    ...
    /// `receiver_wire_destination` uses this for the `c=` line on
    /// synthesis and passthrough — see that helper's doc for the
    /// multicast-file caveat. Ignored on the Sender side (senders
    /// use `source_ip` for the same wire concept).
