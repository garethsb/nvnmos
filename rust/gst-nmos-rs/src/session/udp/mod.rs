...
/// Receiver `c=` wire address from IS-05 properties. Matches
/// nmos-cpp `get_connection_address`: `multicast_ip` when set, else
/// `interface_ip` (unicast reception / caps synthesis).
///
...
pub(super) fn receiver_wire_destination(settings: &CommonSettings) -> &str {
...
pub(super) fn sdp_build_input<'a>(
    ...
    let destination_ip = match settings.side {
        ...
        Side::Receiver => receiver_wire_destination(settings),
    };
...
pub(crate) fn property_overrides_udp(settings: &CommonSettings) -> SdpOverrides<'_> {
    ...
    let (source_ip, interface_ip, destination_ip, source_port) = match settings.side {
        ...
        Side::Receiver => (
            ...
            // unset, [`receiver_wire_destination`] also maps this into
            // `destination_ip` for the `c=` splice — see its doc.
            opt(&settings.interface_ip),
            opt(receiver_wire_destination(settings)),
            None,
        ),
    };
    let tc = settings
