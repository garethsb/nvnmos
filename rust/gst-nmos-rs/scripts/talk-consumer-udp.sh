...
if [[ -z "${DEMO_NIC_IP:-}" || "$DEMO_NIC_IP" == 127.* ]]; then
    echo "talk-consumer-udp: set DEMO_NIC_IP to a non-loopback IPv4 address" >&2
    exit 1
fi
